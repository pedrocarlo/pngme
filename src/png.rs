...
impl Png {
    ...
    // In my opinion this should be an option instead
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        Self::STANDARD_HEADER
            ...
            .chain(
                self.0
                    .iter()
                    .map(|chunk| chunk.as_bytes().into_iter())
                    .flatten(),
            )
            .collect()
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Self::STANDARD_HEADER {
            bail!("header is not the STANDARD_HEADER")
        }
        let mut start = 8;
        let mut png = Vec::new();

        while start < value.len() {
            let chunk = Chunk::try_from(&value[start..])?;
            start += chunk.size();
            png.push(chunk);
        }
